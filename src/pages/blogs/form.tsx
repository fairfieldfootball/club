...
const BlogForm = (props: Props) => (
  <Layout>
    <LayoutBookend>
      <ButtonGroup align="left">
        <Button
          ...
          variant="text"
          padding="none"
          to={props.blog ? urls.blogs.view(props.blog._id.toHexString()) : urls.blogs.list}
        >
          Back
        </Button>
      </ButtonGroup>
    ...
    <FormContainer>
      <Form
        initialData={initialForm(props)}
        onSubmit={data =>
          props.saveBlog(data).then(res => urls.blogs.view((props.blog ? props.blog._id : res).toHexString()))
        }
        validation={{
          type: (value?: BlogType) => {
            if (!value) {
